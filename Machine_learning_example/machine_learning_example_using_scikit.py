...
def clean_data(adata):
    ...
    data['Pickup_dt'] = data.Pickup_dt.apply(lambda x:datetime.datetime.strptime(x,"%Y-%m-%d %H:%M:%S"))
    data['Dropoff_dt'] = data.Dropoff_dt.apply(lambda x:datetime.datetime.strptime(x,"%Y-%m-%d %H:%M:%S"))
...
def test_classification(df, label, yl=[0, 50]):
    ...
    if len(pd.unique(
            df[label])) == 2:  # check if the variable is categorical with only two  categores and run chisquare test
...
# save the best estimator on disk as pickle for a later use
with open(r'D:\OneDrive\Career Development\Job\NTT_Data\my_rfr_reg2.pkl','wb') as fid:
