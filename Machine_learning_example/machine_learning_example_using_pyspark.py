...
from pyspark.sql.functions import mean, min, max, udf, date_format
...
print ('# of columns:  ', data_pd.shape[0])
...
table1 = data_pd.pivot_table(index='Pickup_hour', values='trip_distance',aggfunc=('mean','median')).reset_index()
# rename columns
...
airports_trips = data_pd[(data_pd.RatecodeID==2) | (data_pd.RatecodeID==3)]
...
v3 = data_pd.loc[~data_pd.index.isin(v2.index),'trip_distance'] # non-airport trips
...
data_pd.loc[~data_pd.index.isin(v2.index),'Pickup_hour'].value_counts(normalize=True).sort_index().plot(ax=ax[1])
...
print ("size before feature engineering:", data_pd.shape)
data_pd = engineer_features(data_pd)
print ("size after feature engineering:", data_pd.shape)

## code to compare the two Tip_percentage identified groups
# split data in the two groups
data1 = data_pd[data_pd.tip_percentage>0]
data2 = data_pd[data_pd.tip_percentage==0]

# generate histograms to compare
fig,ax=plt.subplots(1,2,figsize=(14,4))
data_pd.tip_percentage.hist(bins = 20,normed=True,ax=ax[0])
ax[0].set_xlabel('Tip (%)')
...
train = data_pd[(data_pd.Pickup_dt <= '2017-01-31 23:59:59')]  # make a copy of the training set
...
# for predictors start with candidates identified during the exploratory data analysis
