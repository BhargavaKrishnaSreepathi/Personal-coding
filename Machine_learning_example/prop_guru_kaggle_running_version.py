...
for i in range(len(train)):
    ...
    if len(arr.shape) > 2:
        ...
        train_data_original.append(img.flatten())
...
X, Y = create_placeholders(IMG_SIZE*IMG_SIZE*3, len(class_labels))
...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1", [25, IMG_SIZE*IMG_SIZE*3], initializer=tf.contrib.layers.xavier_initializer(seed=1))
    b1 = tf.get_variable("b1", [25, 1], initializer=tf.zeros_initializer())
    W2 = tf.get_variable("W2", [12, 25], initializer=tf.contrib.layers.xavier_initializer(seed=1))
    b2 = tf.get_variable("b2", [12, 1], initializer=tf.zeros_initializer())
    W3 = tf.get_variable("W3", [len(class_labels), 12], initializer=tf.contrib.layers.xavier_initializer(seed=1))
    b3 = tf.get_variable("b3", [len(class_labels), 1], initializer=tf.zeros_initializer())
    ### END CODE HERE ###
...
parameters = model(train_images_T, train_labels_T, train_images_T, train_labels_T)
